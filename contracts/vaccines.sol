...
//         _productPrice, "Available",manufacturingDate);
...
//     function viewProductItems () public view returns(uint256[] memory, bytes32[] memory, bytes32[] memory, bytes32[] memory, uint256[] memory, bytes32[] memory,uint256[] memory) {
...
//         //NEW
//         uint256[] memory pdates = new uint256[](productCount);
...
//             pdates[i] = productItems[i].manufacturingDate;
//         }
//         return(pids, pSNs, pnames, pbrands, pprices, pstatus, pdates);
//     }
...
//                     //NEW
//                     pdates[k] = productItems[i].manufacturingDate;
...
//     function createIdentifier(bytes32 vaccineSN, uint manufacturerId, uint256 manufacturingDate, bytes32 manufactureAddress) public returns (bytes32) {
//         bytes32 identifier = keccak256(abi.encode(vaccineSN, manufacturerId, manufacturingDate, manufactureAddress));
...
//     function verifyIdentifier(bytes32 identifier) public view returns (bool) {
...
contract vaccine {
    ...
    function addProduct(bytes32 _manufactuerID, bytes32 _productSN, bytes32 _productBrand,
    uint256 _productPrice,bytes32 _manufactureAddress) public{
        //NEW 
        uint256 manufacturingDate=block.timestamp;

        productItems[productCount] = productItem(productCount, _productSN, _productBrand,
        _productPrice, "Available",manufacturingDate,_manufactureAddress);
        productMap[_productSN] = productCount;
        productCount++;
        productsManufactured[_productSN] = _manufactuerID;
    }
    
    function getProduct(uint256 _productId) public view returns (productItem memory) {
            return productItems[_productId];
        }

    function viewProductItems () public view returns(uint256[] memory, bytes32[] memory, bytes32[] memory, uint256[] memory, bytes32[] memory,uint256[] memory,bytes32[] memory) {
        uint256[] memory pids = new uint256[](productCount);
        bytes32[] memory pSNs = new bytes32[](productCount);
        // bytes32[] memory pnames = new bytes32[](productCount);
        bytes32[] memory pbrands = new bytes32[](productCount);
        ...
        //NEW
        uint256[] memory pdates = new uint256[](productCount);
        bytes32[] memory paddress = new bytes32[](productCount);
        ...
        for(uint i=0; i<productCount; i++){
            ...
            pstatus[i] = productItems[i].productStatus;
            pdates[i] = productItems[i].manufacturingDate;
            paddress[i] = productItems[i].manufactureAddress;

        }
        return(pids, pSNs, pbrands, pprices, pstatus, pdates,paddress);
    }
    ...
    function queryProductsList(bytes32 _sellerCode) public view returns(uint256[] memory, bytes32[] memory, bytes32[] memory, uint256[] memory, bytes32[] memory,uint256[] memory, bytes32[] memory){
        bytes32[] memory productSNs = productsWithSeller[_sellerCode];
        uint256 k=0;
        ...
        for(uint i=0; i<productCount; i++){
            for(uint j=0; j<productSNs.length; j++){
                if(productItems[i].productSN==productSNs[j]){
                    ...
                    //NEW
                    pdates[k] = productItems[i].manufacturingDate;
                    paddress[k] = productItems[i].manufactureAddress;
    ...
    // event CreatedIdentifier(bytes32 identifier);

    function createIdentifier(bytes32 vaccineSN, uint manufacturerId, uint256 manufacturingDate, bytes32 manufactureAddress) public returns (bytes32) {
        bytes32 identifier = keccak256(abi.encode(vaccineSN, manufacturerId, manufacturingDate, manufactureAddress));
        require(!identifiers[identifier], "Identifier already exists");
        identifiers[identifier] = true;
        // emit CreatedIdentifier(identifier);
        _identifier=identifier;

        return _identifier;
    }
    function displayIdentifier() public view returns(bytes32){
        return _identifier;
    }
    //Verification of Vaccine
